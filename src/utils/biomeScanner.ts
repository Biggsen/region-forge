...
import { findNearestBiome, isSeaBiome, getBiomeCategory } from './biomeColors'

export type BiomeBreakdownEntry = { biome: string; count: number; percentage: number }

export type LandVsSeaBreakdown = { land: number; sea: number }

export type CategoryGroup = { category: string; percentage: number; biomes: BiomeBreakdownEntry[] }

export type GroupedLandVsSea = {
  land: { total: number; groups: CategoryGroup[] }
  sea: { total: number; groups: CategoryGroup[] }
}

export function getGroupedLandVsSea(breakdown: BiomeBreakdownEntry[]): GroupedLandVsSea {
  const landGroups = new Map<string, BiomeBreakdownEntry[]>()
  const seaGroups = new Map<string, BiomeBreakdownEntry[]>()
  for (const entry of breakdown) {
    const category = getBiomeCategory(entry.biome)
    if (isSeaBiome(entry.biome)) {
      const list = seaGroups.get(category) ?? []
      list.push(entry)
      seaGroups.set(category, list)
    } else {
      const list = landGroups.get(category) ?? []
      list.push(entry)
      landGroups.set(category, list)
    }
  }
  const toGroups = (map: Map<string, BiomeBreakdownEntry[]>): { total: number; groups: CategoryGroup[] } => {
    let total = 0
    const groups: CategoryGroup[] = []
    for (const [category, biomes] of map.entries()) {
      const percentage = biomes.reduce((sum, b) => sum + b.percentage, 0)
      total += percentage
      groups.push({ category, percentage, biomes })
    }
    groups.sort((a, b) => b.percentage - a.percentage)
    return { total, groups }
  }
  return { land: toGroups(landGroups), sea: toGroups(seaGroups) }
}

export function getLandVsSeaBreakdown(breakdown: BiomeBreakdownEntry[]): LandVsSeaBreakdown {
  let land = 0
  let sea = 0
  for (const { biome, percentage } of breakdown) {
    if (isSeaBiome(biome)) sea += percentage
    else land += percentage
  }
  return { land, sea }
}
...
/**
 * Samples pixels across the entire map image and returns a biome breakdown.
 * Uses color-to-biome matching. Requires a loaded map image.
 */
export function scanBiomesFullImage(image: HTMLImageElement): BiomeBreakdownEntry[] | null {
  const { width, height } = image
  const canvas = document.createElement('canvas')
  canvas.width = width
  canvas.height = height
  ...
  for (let iy = 0; iy < height; iy += SAMPLE_STEP) {
    for (let ix = 0; ix < width; ix += SAMPLE_STEP) {
      const idx = (iy * width + ix) * 4
      const a = data[idx + 3]
      if (a < 128) continue
